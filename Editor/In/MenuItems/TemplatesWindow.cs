...
namespace VidaFramework.Editor
{
    public class TemplatesWindow
    {
        ...
        private void DrawTemplateLister(Vector2 windowSize,string[] items,int placement = 0,bool checkNext = true)
        {
            ...
            SirenixEditorGUI.BeginBox(GUILayout.Width(170),GUILayout.Height(windowSize.y - 100));
            {

                sliderValue[placement] = GUILayout.BeginScrollView(sliderValue[placement], false, true, GUILayout.Width(170), GUILayout.Height(windowSize.y - 100));
                {
                    GUILayout.BeginVertical(GUILayout.Height(items.Length * 37));
                    {
                        ...
                        {
                            GUI.color = selectedTemplate == item ? Color.gray : Color.white;
                            GUIContent content = new GUIContent(item);
                            content.tooltip = item;
                            GUIStyle customButtonStyle = new GUIStyle(GUI.skin.button);
                            customButtonStyle.normal.background = TextureLoader.GetTexture("button.png");
                            if (GUILayout.Button(content,customButtonStyle,GUILayout.Height(35),GUILayout.Width(150)))
                            {
        ...
        private void RenderItemInfo(Vector2 windowSize,string itemName,int placement)
        {
            ...
            GUILayout.Space(25);
            SirenixEditorGUI.BeginBox(GUILayout.Width(400),GUILayout.Height(400));
            {
                sliderValue[placement] = GUILayout.BeginScrollView(sliderValue[placement], false, true, GUILayout.Width(400), GUILayout.Height(400));
                {
